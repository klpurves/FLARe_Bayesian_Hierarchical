...
model {
  ...
  for (p in 1:nsub){
    ...
    for (t in 1:ntrials){
      ...
      shape1_Plus[t,p] = (VPlus[t,p] * (beta[p,1]) + 1;
      shape1_Minus[t,p] = (VMinus[t,p] * (beta[p,2]) + 1;

      shape2_Plus[t,p] = (1-VPlus[t,p]) * (beta[p,1])+ 1;
      shape2_Minus[t,p] = (1-VMinus[t,p]) * (beta[p,2]) + 1;
