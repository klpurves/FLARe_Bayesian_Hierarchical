...
parameters {
  ...
  real <lower=0,upper=1>Gen[nsub]; // shape 'spikiness' for genrealisation paramter for the CS+
...
model {
  ...
  for (p in 1:nsub){
    ...
    for (t in 1:(ntrials-1)){
      ...
      VPlus[t+1,p]=VPlus[t,p]+alpha[p]*deltaPlus[t,p]*Gen[p]; // value calc CS+
      VMinus[t+1,p]=VMinus[t,p]+alpha[p]*deltaMinus[t,p]*Gen[p]; // ditto CS-
    }
