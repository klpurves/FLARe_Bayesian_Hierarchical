...
// 1 beta value
//added rating consistency (i.e. likelihood of using the same value multiple times)
...
model {
  ...
  for (p in 1:nsub){
    ...
    for (t in 1:ntrials){
      shape1_Plus[t,p] = (1 + exp(-mu[p]*VPlus[t,p] * ((VPlus[t,p] * (1-VPlus[t,p])) / beta[p])));
      shape1_Minus[t,p] = (1+ exp(-mu[p]*VMinus[t,p] * ((VMinus[t,p] * (1-VMinus[t,p])) / beta[p])));
      shape2_Plus[t,p] = (1+ exp(-mu[p]*(1-VPlus[t,p]) * ((VPlus[t,p] * (1-VPlus[t,p])) / beta[p])));
      shape2_Minus[t,p] = (1+ exp(-mu[p]*(1-VMinus[t,p]) * ((VMinus[t,p] * (1-VMinus[t,p])) / beta[p])));
