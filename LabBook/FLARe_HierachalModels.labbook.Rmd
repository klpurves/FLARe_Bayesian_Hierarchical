...
print(" ")
print('Average Rhat')
print(mean(!is.na(summary$summary[,10])))

count <- 1

while (count < (dim(summary$summary)[1]-1)){
  
  print(row.names(summary$summary)[count])
  print(" ")
  print(describe((summary$summary[count:(count+341),1])))
  print(" ")
  count <- count+342
...
save(flare_fit, file=file.path(datadir,'flare_fit_simpleMean'))
...
flare_fit_best <- stan(file = stanfile, data = flare_data, iter=chain_iter,warmup = warm_up, chains = chain_n) #add working dir?

save(flare_fit_best, file=file.path(datadir,'flare_fit_2betaMean'))

traceplot(flare_fit_best,'lp__')

# extract fit data
summary_flare <- summary(flare_fit_best)
...
flare_loglike <- extract_log_lik(flare_fit_best, parameter_name = "loglik", merge_chains = TRUE)

#calculate BIC

FLARe_bic <- bic(ntrials,-colMeans(flare_loglike),3) #number of parameters in that model e.g. 4)
...
flare_data<-list(ntrials=ntrials,nsub=nsub,screamPlus = t(screamPlus), screamMinus= t(screamMinus),ratingsPlus=t(plus_scaled_est),ratingsMinus=t(minus_scaled_est),cdf_scale=cdf_scale)
...
```{stan output.var=F}
// remembering to add this to the paramters block:

real lambda[nsub]

// and this to the model block.

deltaPlus[t,p] = screamPlus[t,p]*lambda[p]-VPlus[t,p]; 
deltaMinus[t,p] = screamMinus[t,p]*lambda[p]-VMinus[t,p];
...
shape1_Plus[t,p] = (1+ exp(-mu*VPlus[t,p] * ((VPlus[t,p] * (1-VPlus[t,p])) / beta[p,1])));
shape2_Plus[t,p] = (1+ exp(-mu*(1-VPlus[t,p]) * ((VPlus[t,p] * (1-VPlus[t,p])) / beta[p,1]);
...
* bin the generated ratings so that they reflect the nature of original scale (i.e. not continuous - 0-1 == 1, 1-2 == 2 etc.)
