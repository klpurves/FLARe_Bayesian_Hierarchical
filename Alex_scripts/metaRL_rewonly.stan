...
model {
  ...
  for (p in 1:nsub){ // run the model for each subject
    ...
    for (t in 1:ntrials-1){
      deltaA[t,p] = (rewardA[t,p]) - QA[t,p]; // prediction error for A
      deltaB[t,p] = (1-rewardA[t,p])- QB[t,p]; // prediction error for B
      QA[t+1,p] = QA[t,p] + rewardA[t,p] *  alpha[p] * deltaA[t,p]; // Q learning for A
      QB[t+1,p] = QB[t,p] + (1-rewardA[t,p]) *  alpha[p] * deltaB[t,p]; // should delta be same in both cases?
    }
    ...
    for (t in 1:ntrials){
      if (includeTrial[t]==1){ // if  we want to fit the trial (we don't have missing responses)
        ...
        // choices[t,p] ~ (exp(QA[t,p]/beta[p])/((exp(QA[t,p]/beta[p])+(QB[t,p]/beta[p]); // could do using bernoulli_logit
         choices[t,p] ~ bernoulli(exp(QA[t,p]/beta[p])/(exp(QA[t,p]/beta[p])+exp(QB[t,p]/beta[p])));
      }
    }
  }
}
