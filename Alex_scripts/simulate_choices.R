simulate_choices <- function(win_alpha,loss_alpha,betawin,betaloss,task){
  ...
  QA_reward <- rep(0.5, nrow(task)+1) #initialise at 0.5 - agnostic towards rewards and losses
  QA_punish <- rep(0.5, nrow(task)+1) #initialise at 0.5 - agnostic towards rewards and losses
  QB_reward <- rep(0.5, nrow(task)+1) #initialise at 0.5 - agnostic towards rewards and losses
  QB_punish <- rep(0.5, nrow(task)+1) #initialise at 0.5 - agnostic towards rewards and losses
  ...
  for (t in 1:nrow(task)){
    ...
    randoms[t]<- runif(1)
    choices[t]<- ifelse(ProbA[t]>randoms[t], 1, 0)
